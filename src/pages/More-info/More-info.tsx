                        ...
                        <Slider {...settings}>
                            ...
                            <div className={"pressure slide-box"}>
                                <div className={"weather-info-box"}>
                                    ...
                                    <p className={"value"}>{currentData.main.pressure}<span className={"metric"}>hPa</span>
                                    </p>
...
const HourWeatherItem = ({item}: any) => {
    return (
        <li>
            <p><strong>{new Date(item.dt_txt).toLocaleTimeString('en-US', {
                hour12: true,
                hour: 'numeric',
                minute: 'numeric'
            })}</strong></p>
            <p>{item.weather[0].main}</p>
            <img alt='thunder' src={`http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`}
                 className={"temp-icon"}/>
